...
def _by_rank_table(left: pd.DataFrame, right: pd.DataFrame, left_name: str, right_name: str) -> pd.DataFrame:
    left_ranked = left.copy().reset_index(drop=True)
    right_ranked = right.copy().reset_index(drop=True)
    rows = min(len(left_ranked), len(right_ranked))

    return pd.DataFrame(
        {
            "rank": range(1, rows + 1),
            f"{left_name}_stack": _stack_id_col(left_ranked.iloc[:rows]),
            f"{left_name}_score": left_ranked.iloc[:rows]["score"].to_numpy(),
            f"{right_name}_stack": _stack_id_col(right_ranked.iloc[:rows]),
            f"{right_name}_score": right_ranked.iloc[:rows]["score"].to_numpy(),
            f"delta_{left_name}_minus_{right_name}_score": left_ranked.iloc[:rows]["score"].to_numpy()
            - right_ranked.iloc[:rows]["score"].to_numpy(),
        }
    )
